...
![Insert image 1.1 here of the last page of search, notice when "last" is selected we only get to page 68]()
...
![Inset image 1.2 here of the naming convention]()
...
*With our exported CSV files in a folder on our machine, open up ArcGIS Pro and sign in with your account. Create a new map project, name and store it in the same folder as the exported CSV files are located. Load up the main base map and open up the geoprocessing toolbox in the Analysis tab.* In the search bar, type in "geocoding." Side note, this processing takes 40 credits per 1,000 addresses geocoded. When buying credits on ESRI's website, 1000 credits cost $120.  
